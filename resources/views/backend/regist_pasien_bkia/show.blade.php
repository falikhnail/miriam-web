    ...
    <div class="flex flex-col justify-center">
        ...
        <div class="flex flex-row items-start">
            <div class="flex flex-1 flex-col my-10 shadow-lg rounded px-6 py-5 bg-white w-full h-full">
                ...
                @endcan
                @include('backend.regist_pasien_bkia.includes.form')
            </div>
...
@push('after-scripts')
    <script>
        const canUpdate = `{{ Auth::user()->can('edit_pasien_bkia') }}` == 1
        const canDelete = `{{ Auth::user()->can('delete_pasien_bkia') }}` == 1
        const canAdd = `{{ Auth::user()->can('add_pasien_bkia') }}` == 1
        ...
        $(document).ready(() => {
            ...
            if (route == 'edit' || route == 'show') {
                ...
                $("#section-title").text("Tambah Pasien BKIA")
                $("#section-subtitle").text("Lengkapi form mandatory untuk menyimpan pasien BKIA")
                $("#info").hide()
                $("#btn-hapus-pasien").hide()

                if (canAdd) {
                    $("#form-pasien").attr('action', '{{ route('backend.pasien.bkia.store') }}')
                }
            }
        })
        ...
        function setEditShowForm(isShow = false) {
            ...
            if (data) {
                ...
                if (!isShow) {
                    if (canUpdate) {
                        const updateRoute = "{{ route('backend.pasien.bkia.update', ':id') }}".replace(":id", data.id);
                    ...
                    if (canDelete) {
                        $("#btn-hapus-pasien").click(() => {
                            showDeleteConfirm(() => {
                                const route = `{{ route('backend.pasien.bkia.destroy', ':id') }}`.replace(
                                    ':id', data.id);
